...
Verification and browser automation are separate decisions. The evidence requirement comes from the property being checked, its risk, and target-repo authority — for UI-affecting changes that includes rendered verification per `visual-verification.md`, which a capable agent or a human may perform interactively. Committing and running a browser automation suite (Playwright, Cypress, or equivalent) is a different choice, justified by repeatability, regression value, or risk — not by the change being frontend, and not by the workflow reaching Build, Testing, or PR. Which framework to use, and whether a given behavior earns an automated spec, is the target repo's tooling decision, not a Harscode requirement. This neither weakens evidence requirements nor bans browser automation (proposal 0028).
...
| Is it an objective runtime/rendered outcome with a known expected result?                                          | Verify with the appropriate executable/rendered tool; a capable agent may perform the check; commit browser automation only where repeatability, regression value, or risk justifies it                                       | Continue |
...
* [ ] Evidence and automation are decided separately: required rendered/runtime evidence comes from the property and its risk, while a committed browser spec is added only when repeatability, regression value, or risk justifies it — never merely because the change is frontend or the workflow reached Testing
