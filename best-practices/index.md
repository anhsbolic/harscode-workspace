...
| go | [go/testing-concurrency.md](go/testing-concurrency.md) | race condition, goroutine, shared state, -race | no | go test -race, scoped per-area and run in the testing-phase gate (not the build loop) + invariant-asserting concurrent tests |
...
| react  | [react/app-router-routing-conventions.md](react/app-router-routing-conventions.md) | app router, route segment, loading.tsx, error.tsx, layout.tsx, route groups, parallel routes, generateMetadata  | no  | Loading/error state placement per segment; layout persistence boundary; route groups vs parallel routes usage criteria; metadata/SEO via generateMetadata |
...
| react  | [react/api-client-centralization.md](react/api-client-centralization.md) | api client, centralized fetch, csrf header, authorization header, credentials include  | no  | Single low-level request function attaches auth token + CSRF header + credentials on every call; no raw fetch() outside it; one 401-retry, not a loop |
| react  | [react/loading-empty-error-state-conventions.md](react/loading-empty-error-state-conventions.md) | loading state, empty state, error state, skeleton, partial failure, error message leak  | no  | Skeleton over spinner; never surface raw backend error text to the user; per-section failure isolation on multi-source pages; empty-state CTA gated by authorization |
...
| react  | [react/ai-prototype-to-production-translation.md](react/ai-prototype-to-production-translation.md) | design tool export, claude design, v0, prototype code, inline style translation, mock data  | no  | Structure/states/copy/a11y transfer directly from an AI design-tool export; styling mechanism, tokens, mock data, and local state must be translated, not copied |
| react  | [react/visual-verification.md](react/visual-verification.md) | visual verification, visual QA, UI verification, rendered UI, screenshot, responsive check, design reference, prototype comparison, layout regression, visual regression, browser inspection, visual state  | no  | Verify rendered UI changes in representative failure-revealing states and contexts; Build may inspect for feedback while Testing owns independent verification; compare approved intent rather than raw pixels |
| react  | [react/testing-automation-boundary.md](react/testing-automation-boundary.md) | eslint rule, lint enforcement, automation, manual review, no-restricted-syntax, jest-axe  | no  | Mechanically detectable checks become automation; objective runtime/rendered outcomes use executable or rendered verification with independent Testing when defined; unresolved subjective intent stays at the human checkpoint |
...
| authz | go/authorization-and-idor.md, go/role-and-privilege-separation.md, laravel/policy-based-authorization.md, laravel/seeder-only-rbac.md |
...
| pii-and-encryption | postgresql/encryption-at-rest.md, postgresql/audit-log-design.md, data-modeling/deletion-vs-anonymization.md |
