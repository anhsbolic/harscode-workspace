...
## 2026-09-11 — Techplan synthesis found a live, currently-exploitable TOCTOU gap in already-shipped code while gathering Interface Contract file:line facts (D01-08-enforcement-otorisasi, koperasiqu-web-app)

**What happened:** Exploration's raw docs (Sniffing Checklist § Risk, `01-invariants-and-permission-matrix.md`) flagged the *shape* of a risk — RN1.1/RN1.2's "ownership validated at assignment time isn't necessarily re-validated at action-execution time" — as a carry-forward concern, citing it as "resolved sebagian" via `lockForUpdate()` being a general recommended pattern elsewhere in the docs. While filling in this techplan's Interface Contract/Implementation Details (`guardrails.md` § 4, reading the actual controller/service code directly for file:line accuracy rather than trusting the raw doc's characterization), the agent found the risk wasn't just a shape to design around going forward — `IssueSetPasswordLinkService::issue()`, already shipped, locks the `users` row for an unrelated reason (preventing duplicate concurrent first-issuance) but never re-checks Stokis ownership inside that lock at all, meaning a Stokis whose responsibility for a member is reassigned away in the moment between the Policy check and the lock can still successfully issue a working password-reset link for that member. This is a real, presently-exploitable gap, not a hypothetical one the new mechanism just needs to avoid introducing.

**Why it was a problem:** the raw exploration doc's "resolved sebagian" framing (accurate as a general statement — `lockForUpdate()` reduces but doesn't eliminate the risk surface) reads differently once a specific already-shipped call site is checked against it directly: "reduces the surface generally" and "this specific service still has zero mitigation" are two different facts, and only reading the actual code line-by-line during techplan synthesis distinguished them. Per `guardrails.md` § 6, this rose to a stop-and-ask case — the agent paused mid-synthesis, presented the finding with the exact file:line evidence, and got an explicit scope decision (fold the fix into this techplan) before writing §7/§10 around it, rather than either silently noting it as "low risk, no action" or silently expanding scope on its own judgment.

**Mitigation:** no rule change proposed this pass — this is the same underlying pattern as the D01-06 entry directly above (a raw doc's characterization of a shared/general concern turning out to need direct verification against the *specific* code the techplan is about to document), not a new structural gap. Logging as a second instance of that pattern, now specifically for Risk-lens findings (not just "does X contract exist") — when a raw exploration doc's Risk lens describes a concern as general/"resolved sebagian"/"mitigated by a general pattern," and techplan synthesis is about to write Interface Contract or Implementation Details for a *specific* call site that risk applies to, actually read that call site's code before assuming the general mitigation covers it. If this exact shape (a general risk framing turning out to hide a specific already-exploitable gap, caught only during techplan's own code-reading pass) recurs on a different repo/task, promote it into an explicit `guardrails.md` addition alongside § 6.